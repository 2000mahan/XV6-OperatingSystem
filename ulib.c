...
int thread_id = clone(stack);
  ...
  // the new stack (remember that stack grows downwards, so it starts at the top of the page)
  uint *stackPointer = (uint*) ((uint) stack) + pageSize);
  stackPointer--;
  *stackPointer = (uint)arg;
  // eip tells the thread where to start executing frm when it returns to userspace
  np->tf->eip = (uint) fcn;
